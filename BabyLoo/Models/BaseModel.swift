...
extension BaseModel {
    ...
    static func all<T>() -> [T] where T: NSManagedObject {
        ...
        let sort = NSSortDescriptor(key: #keyPath(Loo.timestamp), ascending: false)
...
//        dateFormatter.timeZone = NSTimeZone(name: "GMT") as TimeZone?
        ...
        fetchRequest.predicate = NSPredicate(format: "(timestamp >= %@) AND (timestamp <= %@)", startDate, endDate)
